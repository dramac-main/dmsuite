...
export default function SocialMediaPostWorkspace() {
  ...
  const displayWidth = Math.min(420, currentPlatform.width);
  const displayHeight = displayWidth * (currentPlatform.height / currentPlatform.width);

  /* ── Template Previews (Composition thumbnails) ─────────── */
  const templatePreviews = useMemo<TemplatePreview[]>(
    ...
    [config.primaryColor, config.secondaryColor, config.textColor, config.visualIntensity]
  );
  ...
  const leftPanel = (
    <div className="space-y-3">
      ...
      {/* Template Slider — Composition thumbnails */}
      <TemplateSlider
    ...
    <div className="space-y-3">
      ...
      <div className="space-y-2">
        <button
          ...
          className={`w-full flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-xl text-[0.625rem] font-semibold border transition-all ${
            carouselMode
        ...
        {carouselMode && (
          <>
            ...
            <p className="text-[0.5rem] text-gray-400 text-center">
              Slide {activeSlideIndex + 1} of {slides.length}
            </p>
            <button
              onClick={handleExportAllSlides}
              disabled={isExporting}
              className="w-full flex items-center justify-center gap-1 px-3 py-1.5 rounded-xl bg-primary-500 text-gray-950 text-[0.625rem] font-bold hover:bg-primary-400 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              {isExporting ? (
                <>
                  <IconLoader className="size-3 animate-spin" />
      ...
      <div className="flex items-center gap-1 bg-gray-100 dark:bg-gray-800 rounded-xl p-1">
        {(
          [
            ...
            { id: "export", label: "Multi-Export", icon: <IconDownload className="size-3.5" /> },
            { id: "present", label: "Mockups", icon: <IconSmartphone className="size-3.5" /> },
          ] as const
        ).map((tab) => (
          <button
            key={tab.id}
      ...
      {activeTab === "export" && (
        <div className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-3 space-y-3">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-xs font-semibold text-gray-900 dark:text-white">Export All Sizes</h3>
              <p className="text-[0.5625rem] text-gray-400 mt-0.5">
                ...
                {(formats as ExportFormat[]).map((f) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-0.5">
                      ...
                      >
                        {selectedExports.has(f.id) && (
                          <svg className="size-2" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={3}>
                            <polyline points="20 6 9 17 4 12" />
                          </svg>
                        )}
                      </div>
                    </div>
                    <p className="text-[0.5rem] text-gray-400">{f.width}×{f.height}</p>
                  </button>
                ))}
              </div>
            </div>
          ...
          {activeMockup && (
            <div className="space-y-2">
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-1.5 h-8 rounded-lg bg-primary-500 text-gray-950 text-[0.625rem] font-bold hover:bg-primary-400 transition-colors"
              >
                <IconDownload className="size-3" />
                Download Mockup
              </button>
  ...
  const toolbar = (
    <div className="flex items-center gap-1.5">
      ...
      <span className="text-xs text-gray-500">{currentPlatform.width}×{currentPlatform.height}</span>
      {doc.layers.length > 0 && (
        <>
          <span className="text-gray-600">·</span>
      ...
      <StickyCanvasLayout
        ...
        actionsBar={
          <div className="flex items-center gap-2">
            ...
            <button onClick={handleDownloadJpg} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-gray-700 text-gray-300 text-xs font-medium hover:bg-gray-800 transition-colors">
              <IconDownload className="size-3" />JPG
            </button>
            ...
            <button onClick={() => setActiveTab("export")} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-gray-700 text-gray-300 text-xs font-medium hover:bg-gray-800 transition-colors">
              <IconShare className="size-3" />All Sizes
            </button>
          </div>
        }
        leftPanel={leftPanel}
