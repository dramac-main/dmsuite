// =============================================================================
// DMSuite — Sales Book Step 0: Choose Document Type
// Card grid for selecting invoice, quotation, receipt, etc.
// Adapted from invoice StepDocumentType for the blank form designer.
// =============================================================================
...
function IconQuotation({ className = "" }: { className?: string }) {
  return (
    <svg className={className} width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" />
      <polyline points="14 2 14 8 20 8" /><path d="M9 9l2 2 4-4" />
    </svg>
  );
}
...
function IconCreditNote({ className = "" }: { className?: string }) {
  return (
    <svg className={className} width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" />
      <polyline points="14 2 14 8 20 8" /><path d="M15 13H9" />
    </svg>
  );
}
function IconProforma({ className = "" }: { className?: string }) {
  return (
    <svg className={className} width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" />
      <polyline points="14 2 14 8 20 8" />
      <circle cx="12" cy="15" r="3" /><line x1="12" y1="12" x2="12" y2="15" /><line x1="12" y1="15" x2="14" y2="14" />
    </svg>
  );
}
...
const ICON_MAP: Record<string, React.FC<{ className?: string }>> = {
  receipt: IconInvoice, clipboardList: IconQuotation, checkCircle: IconReceipt,
  truck: IconDelivery, arrowDownCircle: IconCreditNote, fileText: IconProforma,
  shoppingCart: IconPurchaseOrder,
};

const TYPE_ACCENTS: Record<SalesDocumentType, string> = {
...
const TYPE_ICON_COLORS: Record<SalesDocumentType, string> = {
...
export default function SBStepDocumentType() {
  const { nextStep } = useSalesBookWizard();
    ...
    <div className="mx-auto max-w-2xl px-4 py-8">
      ...
      >
        ...
        <h2 className="text-xl font-semibold text-gray-100">
          What type of sales book?
        </h2>
        <p className="text-sm text-gray-500 mt-1">
          Choose the document type for your blank form booklet
        </p>
      </motion.div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {SALES_DOCUMENT_TYPES.map((type, i) => {
          ...
          const Icon = ICON_MAP[config.icon] ?? IconInvoice;
          ...
          return (
            <motion.button
              ...
              className={`group relative flex flex-col items-start gap-3 rounded-xl border bg-linear-to-br p-4 text-left transition-all ${TYPE_ACCENTS[type]} ${
                isSelected ? "ring-2 ring-primary-500/50 scale-[1.02]" : "hover:scale-[1.01]"
              }`}
            >
              <div className={`${TYPE_ICON_COLORS[type]} transition-transform group-hover:scale-110`}>
                <Icon />
              </div>
              <div>
              ...
              <span className="text-[10px] font-mono text-gray-600">{config.numberPrefix}-XXXX</span>
              {isSelected && (
                <motion.div
                  layoutId="sb-type-check"
                  className="absolute top-2.5 right-2.5 w-5 h-5 rounded-full bg-primary-500 flex items-center justify-center"
                >
