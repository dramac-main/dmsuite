...
export default function CreditNoteListView() {
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      list = list.filter((cn) => cn.creditNoteNumber.toLowerCase().includes(q) || getClient(cn.clientId).toLowerCase().includes(q));
    }
  ...
  const columns: Column<CreditNote>[] = [
    { key: "number", label: "Credit Note #", render: (cn) => <span className="font-medium text-gray-200">{cn.creditNoteNumber}</span> },
  ...
  return (
    <div className="flex flex-col h-full">
      <PageHeader
        ...
        actions={<Btn variant="primary" size="sm" onClick={() => setView("creditnote-edit", null)}>+ New Credit Note</Btn>}
      />
      ...
      <div className="flex-1 overflow-y-auto px-4 sm:px-6">
        <DataTable data={filtered} columns={columns} onRowClick={(cn) => setView("creditnote-edit", cn.id)} emptyTitle="No credit notes" emptyDescription="Create a credit note to refund or adjust an invoice" />
      </div>
    </div>
  );
}
