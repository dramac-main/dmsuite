// =============================================================================
// DMSuite — Invoice Step 3: Payment & Dates
// Invoice number, dates, payment terms, bank details, notes/terms.
// =============================================================================

"use client";

import { useCallback } from "react";
import { motion } from "framer-motion";
import { useInvoiceWizard } from "@/stores/invoice-wizard";
import { useInvoiceEditor } from "@/stores/invoice-editor";
import { PAYMENT_TERMS, type PaymentTermsId } from "@/lib/invoice/schema";

// ── Icons ──

function IconCreditCard({ className = "" }: { className?: string }) {
  return (
    <svg className={className} width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <rect x="2" y="5" width="20" height="14" rx="2" />
      <line x1="2" y1="10" x2="22" y2="10" />
    </svg>
  );
}

function IconArrowRight({ className = "" }: { className?: string }) {
  return (
    <svg className={className} width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <line x1="5" y1="12" x2="19" y2="12" /><polyline points="12 5 19 12 12 19" />
    </svg>
  );
}

function IconArrowLeft({ className = "" }: { className?: string }) {
  return (
    <svg className={className} width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <line x1="19" y1="12" x2="5" y2="12" /><polyline points="12 19 5 12 12 5" />
    </svg>
  );
}

// ── Shared field ──

function Field({
  label,
  value,
  onChange,
  placeholder,
  type = "text",
  multiline = false,
}: {
  label: string;
  value: string;
  onChange: (v: string) => void;
  placeholder: string;
  type?: string;
  multiline?: boolean;
}) {
  const baseClass =
    "w-full rounded-lg bg-gray-800/50 border border-gray-700/50 px-3 py-2.5 text-sm text-gray-200 placeholder-gray-500 outline-none transition-all focus:border-primary-500/50 focus:ring-1 focus:ring-primary-500/20";

  return (
    <div className="space-y-1.5">
      <label className="text-xs font-medium text-gray-400">{label}</label>
      {multiline ? (
        <textarea
          value={value}
          onChange={(e) => onChange(e.target.value)}
          placeholder={placeholder}
          rows={3}
          className={`${baseClass} resize-none`}
        />
      ) : (
        <input
          type={type}
          value={value}
          onChange={(e) => onChange(e.target.value)}
          placeholder={placeholder}
          className={baseClass}
        />
      )}
    </div>
  );
}

// =============================================================================

export default function StepPayment() {
  const { nextStep, prevStep } = useInvoiceWizard();
  const invoice = useInvoiceEditor((s) => s.invoice);
  const setInvoiceNumber = useInvoiceEditor((s) => s.setInvoiceNumber);
  const setIssueDate = useInvoiceEditor((s) => s.setIssueDate);
  const setDueDate = useInvoiceEditor((s) => s.setDueDate);
  const setPoNumber = useInvoiceEditor((s) => s.setPoNumber);
  const setPaymentTerms = useInvoiceEditor((s) => s.setPaymentTerms);
  const updatePaymentInfo = useInvoiceEditor((s) => s.updatePaymentInfo);
  const setNotes = useInvoiceEditor((s) => s.setNotes);
  const setTerms = useInvoiceEditor((s) => s.setTerms);

  const handlePaymentTermChange = useCallback(
    (termsId: PaymentTermsId) => {
      setPaymentTerms(termsId);
      // Due date auto-computed by store when issueDate is re-set
      if (invoice.issueDate) {
        setIssueDate(invoice.issueDate);
      }
    },
    [setPaymentTerms, invoice.issueDate, setIssueDate]
  );

  return (
    <div className="mx-auto max-w-lg px-4 py-8">
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.1 }}
        className="flex flex-col items-center text-center mb-8"
      >
        <div className="flex items-center justify-center w-14 h-14 rounded-2xl bg-primary-500/10 border border-primary-500/20 mb-4">
          <IconCreditCard className="text-primary-400" />
        </div>
        <h2 className="text-xl font-semibold text-gray-100">Payment Details</h2>
        <p className="text-sm text-gray-500 mt-1">
          Set dates, terms, and how you want to be paid.
        </p>
      </motion.div>

      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.2 }}
        className="space-y-6"
      >
        {/* Invoice Meta */}
        <section className="space-y-3">
          <h3 className="text-xs font-medium text-gray-500 uppercase tracking-wider">Invoice Details</h3>
          <div className="grid grid-cols-2 gap-3">
            <Field
              label="Invoice Number"
              value={invoice.invoiceNumber}
              onChange={setInvoiceNumber}
              placeholder="INV-001"
            />
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-gray-400">Payment Terms</label>
              <select
                value={invoice.paymentTerms}
                onChange={(e) =>
                  handlePaymentTermChange(e.target.value as PaymentTermsId)
                }
                className="w-full rounded-lg bg-gray-800/50 border border-gray-700/50 px-3 py-2.5 text-sm text-gray-200 outline-none focus:border-primary-500/50"
              >
                {PAYMENT_TERMS.map((t) => (
                  <option key={t.id} value={t.id}>
                    {t.label}
                  </option>
                ))}
              </select>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <Field
              label="Issue Date"
              value={invoice.issueDate}
              onChange={setIssueDate}
              placeholder="YYYY-MM-DD"
              type="date"
            />
            <Field
              label="Due Date"
              value={invoice.dueDate}
              onChange={setDueDate}
              placeholder="YYYY-MM-DD"
              type="date"
            />
          </div>
          <Field
            label="P.O. Number (optional)"
            value={invoice.poNumber}
            onChange={setPoNumber}
            placeholder="PO-2024-001"
          />
        </section>

        {/* Payment Info */}
        <section className="space-y-3">
          <h3 className="text-xs font-medium text-gray-500 uppercase tracking-wider">Bank / Payment Info</h3>
          <div className="grid grid-cols-2 gap-3">
            <Field
              label="Bank Name"
              value={invoice.paymentInfo.bankName}
              onChange={(v) => updatePaymentInfo({ bankName: v })}
              placeholder="First National Bank"
            />
            <Field
              label="Account Name"
              value={invoice.paymentInfo.accountName}
              onChange={(v) => updatePaymentInfo({ accountName: v })}
              placeholder="Acme Corp"
            />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <Field
              label="Account Number"
              value={invoice.paymentInfo.accountNumber}
              onChange={(v) => updatePaymentInfo({ accountNumber: v })}
              placeholder="1234567890"
            />
            <Field
              label="Routing / Sort Code"
              value={invoice.paymentInfo.routingNumber}
              onChange={(v) => updatePaymentInfo({ routingNumber: v })}
              placeholder="021000021"
            />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <Field
              label="SWIFT / BIC Code"
              value={invoice.paymentInfo.swiftCode}
              onChange={(v) => updatePaymentInfo({ swiftCode: v })}
              placeholder="BOFAUS3N"
            />
            <Field
              label="PayPal Email"
              value={invoice.paymentInfo.paypalEmail}
              onChange={(v) => updatePaymentInfo({ paypalEmail: v })}
              placeholder="payments@company.com"
            />
          </div>
        </section>

        {/* Notes & Terms */}
        <section className="space-y-3">
          <h3 className="text-xs font-medium text-gray-500 uppercase tracking-wider">Notes & Terms</h3>
          <Field
            label="Notes"
            value={invoice.notes}
            onChange={setNotes}
            placeholder="Thank you for your business!"
            multiline
          />
          <Field
            label="Terms & Conditions"
            value={invoice.terms}
            onChange={setTerms}
            placeholder="Payment is due within 30 days of receipt."
            multiline
          />
        </section>
      </motion.div>

      {/* Navigation */}
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ delay: 0.3 }}
        className="flex justify-between mt-8"
      >
        <button
          onClick={prevStep}
          className="inline-flex items-center gap-2 rounded-lg px-4 py-2.5 text-sm font-medium text-gray-400 hover:text-gray-200 transition-colors"
        >
          <IconArrowLeft className="w-4 h-4" />
          Back
        </button>
        <button
          onClick={nextStep}
          className="inline-flex items-center gap-2 rounded-lg bg-primary-500 px-5 py-2.5 text-sm font-medium text-gray-950 transition-all hover:bg-primary-400 active:scale-[0.98]"
        >
          Continue
          <IconArrowRight className="w-4 h-4" />
        </button>
      </motion.div>
    </div>
  );
}
