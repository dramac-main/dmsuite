// =============================================================================
// Business Plan Style Tab — Templates, accent color (HSV), fonts, headers
// =============================================================================
...
// ─── HSV Color Picker (matches Contract/Sales pattern) ───
...
function TemplateMiniPreview({ tpl, selected, onClick }: {
  tpl: (typeof BUSINESS_PLAN_TEMPLATES)[number];
  selected: boolean;
  onClick: () => void;
}) {
  return (
    ...
    >
      ...
      <div className="w-full h-20 rounded-lg overflow-hidden bg-white mb-2">
        ...
        <div className="p-2">
          ...
          <div className="w-1/2 h-1.5 rounded-full mb-0.5" style={{ backgroundColor: tpl.accent, opacity: 0.5 }} />
          <div className="w-full h-1 rounded-full bg-gray-200 mb-0.5" />
          <div className="w-4/5 h-1 rounded-full bg-gray-200" />
        </div>
      </div>
      <div className="text-xs font-semibold text-gray-200">{tpl.name}</div>
      <div className="text-[10px] text-gray-500 leading-tight">{tpl.description}</div>
    </button>
  );
}
...
export default function BusinessPlanStyleTab() {
  ...
  return (
    <div className="space-y-2">
      ...
      <AccordionSection
        title="Template"
        icon={icons.template}
        badge={BUSINESS_PLAN_TEMPLATES.find((t) => t.id === store.form.style.template)?.name}
      ...
      >
        <FormSelect
          ...
          onChange={(e) => store.updateStyle({ fontPairing: e.target.value })}
        >
          {fontOptions.map((fo) => (
      ...
      >
        ...
        {/* Cover style */}
        <div className="mt-3">
          <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-2 block">Cover Page Style</span>
          <ChipGroup
          options={coverStyleOptions}
          value={store.form.style.coverStyle}
          onChange={(v) => store.updateStyle({ coverStyle: v as StyleConfig["coverStyle"] })}
            direction="grid"
            columns={2}
          />
        </div>
      ...
      >
        <div className="space-y-2">
          <Toggle
            label="Cover Page"
            description="Show a styled cover page"
            checked={store.form.style.showCoverPage}
            onChange={(v) => store.updateStyle({ showCoverPage: v })}
          />
          <Toggle
            label="Table of Contents"
            description="Auto-generated section listing"
            checked={store.form.style.showTableOfContents}
            onChange={(v) => store.updateStyle({ showTableOfContents: v })}
          />
          <Toggle
            label="Page Numbers"
