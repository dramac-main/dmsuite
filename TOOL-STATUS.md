...
| # | Tool ID | Tool Name | Workspace | Notes |
|---|---------|-----------|-----------|-------|
| 22 | `ai-chat` | AI Chat Assistant | ai-chat/AIChatWorkspace | **V3 Lobe Chat-inspired rebuild + Vercel AI SDK.** 6 AI models (Claude Sonnet 4/Haiku 4, GPT-4o/mini, Gemini 2.0 Flash, DeepSeek V3) across 4 providers (claude/openai/gemini/deepseek). **Powered by Vercel AI SDK** (`ai` v6, `@ai-sdk/anthropic`, `@ai-sdk/openai`, `@ai-sdk/google` — all Apache-2.0). Unified `streamText()` API route. Agent persona system (6 built-in + custom). Topic threading. Zustand+Immer+persist store (~560 lines). Conversation CRUD (create/delete/rename/pin/archive/duplicate). Message actions (copy/edit/bookmark/fork/regenerate). Full markdown rendering (ReactMarkdown+remarkGfm+remarkMath+rehypeHighlight+rehypeKatex). Code blocks with language header+copy. File upload (images+docs). Agent selector modal. Model selector dropdown. Conversation sidebar with search, pinned/recent sections. Export (json/markdown/text) + import. Chiko AI manifest (12 actions + activity logging). Store adapter. Credit system (5cr/message). 0 TS errors. |
| 23 | `ai-chat-v2` | AI Chat V2 (Lobe) | ai-chat-v2/AIChatV2Workspace | **Lobe Chat–faithful 3-panel chat.** Same 6 models, 4 providers as V3. **Key differences from V3:** Session-based (vs Conversation), Session Groups with collapsible sections, Topic threading per session, 10 built-in agents (vs V3's 6: LobeChat Assistant/Creative Muse/Code Pilot/Data Scientist/Strategy Advisor/UX Architect/Copy Chief/DevOps Engineer/Language Tutor/Research Analyst). Per-session model/temperature/maxTokens/systemPrompt overrides. 3-panel layout: Session sidebar (left, 72w, with groups/pinned/search) + Chat area (center) + toggleable right panel (Topic panel 60w OR Settings panel 80w with agent market tab). Rich message actions (copy/edit/bookmark/fork/regenerate/translate/TTS). Model selector dropdown with provider groups. Empty state with featured agents grid + suggestion chips. Zustand+Immer+persist store (~850 lines). Chiko manifest (13 actions: state/session CRUD/model/agent/topics/export/reset + activity logging). 0 TS errors. |
