...
| # | Tool ID | Tool Name | Workspace | Notes |
|---|---------|-----------|-----------|-------|
| 22 | `ai-chat` | AI Chat Assistant | ai-chat/AIChatWorkspace | **V3 Lobe Chat-inspired rebuild.** 6 AI models (Claude Sonnet 4/Haiku 4, GPT-4o/mini, Gemini 2.0 Flash, DeepSeek V3) across 4 providers (claude/openai/gemini/deepseek). Agent persona system (6 built-in + custom). Topic threading. Zustand+Immer+persist store (~560 lines). Conversation CRUD (create/delete/rename/pin/archive/duplicate). Message actions (copy/edit/bookmark/fork/regenerate). Multi-provider streaming API (/api/chat). Full markdown rendering (ReactMarkdown+remarkGfm+remarkMath+rehypeHighlight+rehypeKatex). Code blocks with language header+copy. File upload (images+docs). Agent selector modal. Model selector dropdown. Conversation sidebar with search, pinned/recent sections. Export (json/markdown/text) + import. Chiko AI manifest (12 actions + activity logging). Store adapter. Credit system (5cr/message). 0 TS errors. |
